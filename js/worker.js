...
importScripts("constants.js", "crypto-js.min.js", "enc-hex.min.js");
...
function HW_AES_AscUnvisible(encryptedStr) {
  ...
  for (let i = 0; i < buf.length; i++) {
    if (0x7e == buf[i])
      // character ~
      buf[i] = 0x1e;
    else buf[i] = buf[i] - 0x21; // chracter !
  }
  return buf;
}
...
function HW_AES_AscVisible(buffer) {
  ...
  for (let i = 0; i < buffer.length; i++) {
    ...
    if (encodedChar == 0x1e) encodedChar = 0x7e; // char ~
    else encodedChar += 0x21;
...
function HW_AES_PlainToBin(buffer) {
  ...
  for (let i = 0; i != output.length; i += 4) {
    let _long = HW_AES_AesEnhSysToLong(
      buffer.slice(periodFive, periodFive + 5)
    );
...
function HW_AES_Trim(encryptedStr) {
  ...
  if (
    "$" != encryptedStr[0] ||
    "2" != encryptedStr[1] ||
    "$" != encryptedStr[encryptedStr.length - 1]
  ) {
    return "";
  }
...
function HW_AESCBC_Decrypt(input, key) {
  if (!input instanceof Uint8Array && typeof input != "string") return "";
  if (!key instanceof Uint8Array && typeof key != "string") return "";
  if (key instanceof Uint8Array) key = toHexString(key);

  let decrypted = "";
  let unvisible = HW_AES_AscUnvisible(HW_AES_Trim(input));
  let blockCount = (unvisible.length / BLOCK_SIZE) >> 0;
  if (unvisible.length != BLOCK_SIZE * blockCount) return "";

  let IV = (Global_IV = HW_AES_PlainToBin(
    unvisible.slice(
      blockCount * BLOCK_SIZE - BLOCK_SIZE,
      blockCount * BLOCK_SIZE
    )
  ));
  const dataAll = HW_AES_PlainToBin(
    unvisible.slice(0, blockCount * BLOCK_SIZE - BLOCK_SIZE)
  );
...
function HW_AESCBC_Encrypt(data, key, IV) {
  ...
  if (!key instanceof Uint8Array && typeof key != "string") return "";
  if (!IV instanceof Uint8Array && typeof IV != "string") return "";
  if (key instanceof Uint8Array) key = toHexString(key);
  if (IV instanceof Uint8Array) IV = toHexString(IV);
...
onmessage = function (e) {
  if (e == null || e.data == null || e.data.functionCall == null) return;
  let returnedData = {
  ...
  switch (e.data.functionCall) {
    case GENERATE_PASSWORD:
      returnedData.return = GeneratePassword(
        e.data.args.password,
        e.data.args.salt,
        e.data.args.mode
      );
      break;
    case ENCRYPT_DATA:
      returnedData.return = HW_AESCBC_Encrypt(
        e.data.args.password,
        PASSWORD_HEX,
        GetRandomByteArray(0x10)
      );
      returnedData.field = e.data.args.field;
      break;
    case DECRYPT_DATA:
      returnedData.return = HW_AESCBC_Decrypt(
        e.data.args.password,
        PASSWORD_HEX
      ); // HW_AESCBC_Encrypt(e.data.args.password, PASSWORD_HEX, GetRandomByteArray(0x10));
      returnedData.field = e.data.args.field;
      break;
  }
